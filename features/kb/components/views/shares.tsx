  ...
  return (
    <div className="flex items-start gap-3 rounded-2xl border border-hk-border bg-hk-card p-4">
      ...
      <div className="min-w-0 flex-1">
        <button
          className="block w-full truncate text-left text-[15px] font-semibold tracking-tight text-hk-text"
          onClick={() => router.push(`/search${hashHref("doc", share.path)}`)}
        >
          {title}
        </button>
        <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-hk-faint">
          ...
          {expiry && (
            <>
              <span className="h-[3px] w-[3px] rounded-full bg-hk-faint" />
              <span className={expiry.expired ? "font-semibold text-hk-orange-text" : ""}>
                {expiry.text}
              </span>
            </>
          )}
        ...
        {share.url ? (
          <div className="mt-2 flex items-center gap-2">
            ...
            <button
              className="flex shrink-0 items-center gap-1 rounded-lg border border-hk-border px-2 py-1 text-[11.5px] font-semibold text-hk-text-2 transition-colors hover:bg-hk-pill"
              onClick={copy}
            >
          ...
          <div className="mt-2 text-[11.5px] text-hk-weak">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto">
      <div className="mx-auto w-full max-w-2xl px-4 py-5 pb-[max(env(safe-area-inset-bottom),24px)]">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h1 className="text-[21px] font-bold tracking-tight text-hk-heading">Shared notes</h1>
            <p className="mt-1 text-[12.5px] text-hk-weak">
              Served live from your home machine — nothing is stored anywhere else.
            </p>
          </div>
          {shares.length > 0 && (
            <span className="mt-1 rounded-full bg-hk-pill px-2 py-0.5 text-[11.5px] font-semibold text-hk-text-2 tabular-nums">
              {shares.length}
            </span>
          )}
        </div>
        ...
        {loading && !loaded ? (
          ...
          <div className="mt-10 flex flex-col items-center gap-2 py-8 text-center">
            <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-hk-glyph text-hk-coral-text">
              <IconShare size={19} strokeWidth={1.5} />
            </span>
            <p className="text-[14.5px] font-semibold text-hk-text">No active shares</p>
            <p className="max-w-[300px] text-[12.5px] leading-relaxed text-hk-weak">
