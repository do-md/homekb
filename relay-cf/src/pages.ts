 ...
 * Port of relay/src/pages.ts with the client lookup made async (D1).
...
export async function renderAuthorizePage(
  ...
  const client = clientId
    ? await env.DB.prepare(
        "SELECT client_id, redirect_uris, name FROM oauth_clients WHERE client_id = ?",
      )
        .bind(clientId)
        .first<{ client_id: string; redirect_uris: string; name: string }>()
    : null;
